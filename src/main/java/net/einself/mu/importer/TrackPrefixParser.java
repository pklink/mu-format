...
public class TrackPrefixParser {
    ...
    private static final Pattern DISC_AND_NUMBER =
            Pattern.compile("^(\\d{1,2})[-.](\\d{1,3})" + SEPARATOR + "(.*)$");

    /** {@code A1 Title}, {@code A-01 Title} — medium side and track. */
    private static final Pattern SIDE_AND_NUMBER =
            Pattern.compile("^([A-Za-z])-?(\\d{1,3})" + SEPARATOR + "(.*)$");

    /** {@code 01 Title} — track only. */
    private static final Pattern NUMBER_ONLY =
            Pattern.compile("^(\\d{1,3})" + SEPARATOR + "(.*)$");
    ...
    private static Optional<TrackPosition> position(Object disc, String number, String title, String stem) {
        ...
        if (parsed < 1) {
            // SPEC.md section 4.7 requires number >= 1; "00 Intro" carries no usable position.
            return Optional.empty();
        }
